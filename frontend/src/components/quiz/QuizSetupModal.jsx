                ...
                return (
                    <div className="mt-4">
                        ...
                        <button
                            ...
                            className={`w-full flex items-center justify-center px-4 py-3 border-2 border-dashed rounded-lg transition-all hover:bg-slate-50 ${errors.sourceValue ? 'border-red-300 bg-red-50' : 'border-slate-300'
                                }`}
                        >
                            {pdfFile ? (
                ...
                <div className="grid grid-cols-2 gap-3">
                    {sourceOptions.map(opt => (
                        <button
                            ...
                            className={`relative flex flex-col items-start p-4 rounded-lg border-2 transition-all text-left ${config.sourceType === opt.value
            ...
            <div className="space-y-4">
                <div>
                    ...
                    <input
                        ...
                        className={`w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all ${errors.topic ? 'border-red-300 bg-red-50' : 'border-slate-200'
                            }`}
                    ...
                    <div className="flex justify-center gap-2">
                        {questionCounts.map(count => (
                            <button
                                ...
                                className={`px-4 py-2 rounded-lg font-medium text-sm transition-all ${config.questionCount === count
                ...
                <div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        {questionTypesData.map(type => (
                            <label
                                key={type.value}
                                className={`relative flex items-center gap-3 p-3 rounded-lg border-2 cursor-pointer transition-all ${config.questionTypes.includes(type.value)
                                        ? 'border-amber-400 bg-amber-50'
                                        : 'border-slate-200 hover:border-slate-300 bg-white'
                                    }`}
                            >
                    ...
                    <div id="advanced-settings" className="border-t border-slate-200 p-4 space-y-4">
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                                {difficultyLevels.map(level => (
                                    <button
                                        ...
                                        className={`py-2 px-3 rounded-lg font-medium text-sm transition-all ${config.difficulty === level.value
                                                ? 'bg-amber-100 text-amber-700 border-2 border-amber-400'
                                                : 'bg-white border-2 border-slate-200 text-slate-600 hover:border-slate-300'
                                            }`}
                        ...
                        <div className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                            ...
                            <button
                                ...
                                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${config.immediateFeedback ? 'bg-amber-600' : 'bg-slate-300'
                                    }`}
                            >
                                <span className="sr-only">Enable immediate feedback</span>
                                <span
                                    className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${config.immediateFeedback ? 'translate-x-6' : 'translate-x-1'
                                        }`}
                                />
                            </button>
                        </div>
                            ...
                            {[1, 2].map((step) => (
                                <React.Fragment key={step}>
                                    <div className={`flex items-center gap-2 ${currentStep >= step ? 'text-amber-600' : 'text-slate-400'}`}>
                                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold transition-colors ${currentStep >= step ? 'bg-amber-100 text-amber-600' : 'bg-slate-100 text-slate-400'
                                            }`}>
                                            {currentStep > step ? (
                                    ...
                                    {step < 2 && (
                                        <div className={`flex-1 h-0.5 transition-colors ${currentStep > step ? 'bg-amber-200' : 'bg-slate-200'
                                            }`} />
                                    )}
                                </React.Fragment>
                            ))}
