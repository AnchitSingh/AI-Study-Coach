...
const Tooltip = ({ children, content }) => {
    ...
    return (
        ...
        >
            {children}
            <div
                className={`absolute top-full left-1/2 transform -translate-x-1/2 mt-2 px-2 py-1 bg-gray-800 text-white text-xs rounded-md shadow-lg transition-opacity duration-200 z-50 whitespace-nowrap ${isVisible ? 'opacity-100 visible' : 'opacity-0 invisible'
                    }`}
            >
                {content}
        ...
        <>
            <header className="fixed h-[4rem] top-4 left-1/2 -translate-x-1/2 z-50 w-[95vw] max-w-max">
                ...
                <div className="bg-white/80 backdrop-blur-lg h-full rounded-[1.25rem] shadow-lg flex items-center justify-between p-2">
                    {/* Logo - always visible with "Coach" text only on medium screens and larger */}
                    <a
                        href="#home"
                        onClick={(e) => {
                            e.preventDefault();
                        ...
                        aria-label="Education Coach - Home"
                    >
                        ...
                        <span className="font-bold text-xl text-gray-800 hidden sm:block mr-2">Coach</span>
                    </a>
                            ...
                            {navigationItems.map((item) => {
                                ...
                                return (
                                    <button
                                        ...
                                        aria-current={isActive ? 'page' : undefined}
                                        className={`flex items-center space-x-3 px-4 py-3 rounded-lg text-left text-base font-medium transition-colors ${isActive
