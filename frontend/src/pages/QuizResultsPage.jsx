...
const QuizResultsPage = ({ results, onNavigate }) => {
  ...
  useEffect(() => {
    ...
    const generateFeedback = async () => {
      try {
        ...
        const stream = await aiStudyCoachAPI.getQuizFeedback(quizMeta, stats);
  ...
  useEffect(() => {
    ...
    const fetchRecommendedTopics = async () => {
      try {
        const statsResponse = await aiStudyCoachAPI.getGlobalStats('all');
        ...
        if (statsResponse && statsResponse.success && statsResponse.data) {
          ...
          if (Array.isArray(topicPerformance.weak) && topicPerformance.weak.length > 0) {
            ...
            Array.isArray(topicPerformance.moderate) &&
            topicPerformance.moderate.length > 0
          ) {
  ...
  const renderSubjectiveAnswer = (question, answer) => {
    ...
    if (Array.isArray(textAnswer)) {
      ...
      const userAnswerSentence = parts
        .map((part, index) => {
          if (/_{3,}/.test(part)) {
      ...
      if (!answer.isCorrect && Array.isArray(correctAnswers) && correctAnswers.length > 0) {
        ...
        correctAnswerSentence = question.question.replace(
          /_{3,}/,
          `<strong class="text-green-700 font-semibold">${escapeHtml(question.answer)}</strong>`
        );
      }
        ...
        <div className="space-y-3">
          ...
          {answer.aiEvaluated && (
            ...
            >
              ...
              >
                ✨ AI Evaluation:{' '}
                {answer.feedback?.message || (answer.isCorrect ? 'Correct' : 'Incorrect')}
              </p>
              <p
                className={`text-sm ${
              ...
              >
                {answer.feedback?.explanation || answer.explanation || 'No feedback provided.'}
              </p>
            </div>
          )}
  ...
  const currentQuestion = useMemo(() => {
    ...
    if (
      ...
      currentQuestionIndex >= validatedResults.quiz.questions.length
    ) {
      return null;
    }
      ...
      <main className="relative">
              ...
              <div className="flex flex-col items-center">
                <div className="relative inline-flex items-center justify-center mb-6">
                  ...
                  <div className="relative bg-white rounded-full w-48 h-48 shadow-2xl flex flex-col items-center justify-center border border-amber-100/50">
                    <div
                      className={`text-6xl font-bold bg-gradient-to-r ${performance.color} bg-clip-text text-transparent animate-scale-in`}
                    >
                      {percentage}%
                    </div>
                ...
                <h2 className="text-3xl font-bold">
                  <span
                    className={`bg-gradient-to-r ${performance.color} bg-clip-text text-transparent`}
                  >
                    {performance.text}
                  </span>
                </h2>
        ...
        <div className="lg:hidden px-4 sm:px-6 pb-6 pt-24 space-y-6">
          ...
          <div className="flex flex-col items-center">
            <div className="relative inline-flex items-center justify-center mb-4">
              ...
              <div className="relative bg-white rounded-full w-36 h-36 sm:w-44 sm:h-44 shadow-xl flex flex-col items-center justify-center border border-amber-100/50">
                <div
                  className={`text-4xl sm:text-5xl font-bold bg-gradient-to-r ${performance.color} bg-clip-text text-transparent`}
                >
                  {percentage}%
                </div>
          ...
          >
            <span className="flex items-center justify-center">
              <svg
                className="w-4 h-4 sm:w-5 sm:h-5 mr-2 text-amber-600 group-hover:scale-110 transition-transform duration-300"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-lg border border-white/50">
            <h2 className="text-lg sm:text-xl font-bold text-slate-800 mb-3 flex items-center">
              <svg
                className="w-5 h-5 sm:w-6 sm:h-6 mr-2 text-amber-600"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
          ...
          {currentQuestion ? (
            <div className="space-y-4 max-w-full overflow-hidden">
              ...
              {currentAnswer?.unanswered ? (
                ...
                <div className="mb-6">{renderSubjectiveAnswer(currentQuestion, currentAnswer)}</div>
              ) : (
                <div className="space-y-3 mb-6">
                  {Array.isArray(currentQuestion.options) &&
                    currentQuestion.options.map((option, optionIndex) => {
                      ...
                      const isCorrectOption = Boolean(option.isCorrect);
                      const optionText =
                        typeof option.text === 'string' ? option.text : `Option ${optionIndex + 1}`;
