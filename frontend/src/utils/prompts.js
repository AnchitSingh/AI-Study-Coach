...
GOOD (corrected form):
{
  ...
  const examples = finalTypes.flatMap((type) => getExampleForType(type)).join('\n\n');
...
export function buildStoryPrompt({ extractedSource, config }) {
  ...
  const { storyStyle = 'Simple Words', topic = title || 'the selected topic' } = config || {};
...
export function buildEvaluatePrompt({ question, canonical, userAnswer }) {
  const questionText = question?.text || question?.question || question?.stem || '';

  const referenceAnswer =
    question?.expected_answer || question?.answer || question?.correctAnswer || canonical || '';
