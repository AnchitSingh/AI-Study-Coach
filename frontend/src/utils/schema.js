...
export function validateQuiz(obj) {
  ...
  for (let i = 0; i < obj.questions.length; i++) {
    ...
    if (q.type === 'MCQ' || q.type === 'True/False') {
      ...
      for (let j = 0; j < q.options.length; j++) {
        ...
        if (!isBool(option.isCorrect)) {
          console.error(
            `Quiz validation failed: question ${i} option ${j} missing isCorrect boolean`
          );
          return false;
        }
    ...
    if (q.type === 'Fill in Blank' || q.type === 'Short Answer' || q.type === 'Subjective') {
      if (!q.answer) {
        console.warn(
          `Question ${i} (${q.type}) is missing model answer - AI may struggle to evaluate`
        );
...
export function validateEvaluation(obj) {
  ...
  if (!isString(obj.explanation) && !isString(obj.rationale)) {
    console.error(
      'Evaluation validation failed: explanation or rationale is required and must be a string'
    );
    return false;
  }
